...
public class OrderRepositoryImpl implements OrderRepositoryCustom {
    ...
    public List<Order> selectOrders() {
        ...
        return queryFactory.select(qOrder).from(qOrder).fetch();
    }

    @Override
    public Order selectOrder(int orderId) {
        return queryFactory.selectFrom(qOrder).where(qOrder.orderId.eq(orderId)).fetchOne();
    }

    @Override
    public List<Order> searchOrder(int priceCondition) {
        return List.of();
    }
}
