...
public class SecurityConfig {
    /*
        ...
        - 기본 인증(로그인, 로그아웃)은 Security가 제공하는 기본 인증 페이지 동작
        - REST API 기반 인증에서는 토큰방식을 이용하기 때문에 로그인, 로그아웃 발생

        인가 설정
        - MyUserDetails의 사용자 권한 목록을 제공하는 getAuthorities()에서 반드시 접두어로 ROLE_ 붙여야 됨
        - ROLE_ 접두어를 안붙이면 hasAuthority(), hasAnythority()로 권한 설정
        - 존재하지 않은 요청 주소에 대해서 시큐리티 로그인 페이지로 기본 redirect 수행하기 떄문에 마지막에 anyRequest().permitAll() 권한 설정
